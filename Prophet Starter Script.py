...
# --- NAMES RESTORED (Mapped to IMPS codes) ---
cut_options = {
    "90% Trimmings (90)": "90",
    "81% Trimmings (81)": "81",
    "Striploin (180)": "180",
...
target_promo_price = st.sidebar.number_input("Promo Target Price ($/lb)", value=0.00, step=0.25, format="%.2f", help="Set to 0 to disable. If set, draws a target execution line on the forecast chart.")
        ...
        if resp.status_code == 200 and 'results' in resp.json():
            ...
            if not temp_df.empty:
                ...
                if desc_col in temp_df.columns:
                    matched = temp_df[temp_df[desc_col].astype(str).str.contains(item_code, case=False, na=False, regex=True)].copy()
...
@st.cache_data(show_spinner="Training AI...")
def run_prophet(_df, price_col, weeks_out, item_code):
...
if api_key:
    ...
    if not df_historical.empty:
        model, forecast = run_prophet(df_historical, target_price_col, forecast_weeks, target_code)
        ...
        actual_df = df_historical[['report_date', target_price_col]].copy()
        pred_df = forecast[['ds', 'yhat']].copy()
        merged_df = pd.merge(actual_df, pred_df, left_on='report_date', right_on='ds', how='inner')
        r_squared = r2_score(merged_df[target_price_col], merged_df['yhat'])

        CORRELATION_THRESHOLD = 0.60
        data_is_reliable = r_squared >= CORRELATION_THRESHOLD
        
        if not data_is_reliable:
            st.error(f"🚨 **WARNING: NON-CORRELATED DATA (R-Squared: {r_squared:.2f})**")
            st.warning(f"The AI cannot find a reliable historical pattern for **{selected_cut_name}**. The predictive data below is deemed unreliable and should not be used for major purchasing decisions.")
            signal_color = "gray"
            signal_text = "DATA UNRELIABLE"
        else:
            st.success(f"✅ **Strong Correlation (R-Squared: {r_squared:.2f})** - Predictive data is reliable.")
            if margin > (current_price * 0.05): 
                signal_color = "green"
                signal_text = "BUY NOW"
            elif margin < -(current_price * 0.03): 
                signal_color = "red"
                signal_text = "SELL / HOLD OFF"
            else:
                signal_color = "orange"
                signal_text = "FLAT - WATCH MARKET"
        ...
        with col2:
            st.metric(f"Predicted Price (in {forecast_weeks} weeks)", f"${predicted_price:.2f}/lb", delta=f"${(predicted_price - current_price):.2f}")
        with col3:
            st.markdown(f"<h3 style='text-align: center; color: {signal_color}; border: 2px solid {signal_color}; padding: 10px;'>SIGNAL: {signal_text}</h3>", unsafe_allow_html=True)
        ...
        # --- NEW FEATURE: TARGET EVENT REVERSE CALCULATOR ---
        ...
        # Get future Fridays from the Prophet forecast
        future_dates = forecast[forecast['ds'] > pd.to_datetime('today')]['ds'].dt.strftime('%Y-%m-%d').tolist()
        
        if future_dates and data_is_reliable:
            target_date_str = st.selectbox("When do you need the product in-house?", future_dates, index=len(future_dates)-1)
            target_date = pd.to_datetime(target_date_str)
            
            # Filter the forecast down to the exact window we have to act
            actionable_window = forecast[(forecast['ds'] > pd.to_datetime('today')) & (forecast['ds'] <= target_date)].copy()
            
            if not actionable_window.empty:
                # Math: Calculate holding cost to keep it in the freezer from the buy date to the target date
                ...
                actionable_window['total_landed_cost'] = actionable_window['yhat'] + freight_cost + actionable_window['holding_cost']
                
                # Find the week with the absolute lowest Landed Cost
                ...
                # Calculate what it would cost to just buy it right now today
        ...
        with tab1:
            ...
            # --- NEW FEATURE: Target Price H-Line ---
            ...
            fig.update_layout(xaxis_title="Date", yaxis_title="Price ($/lb)", hovermode="x unified")
        ...
        with tab2:
            ...
            if baseline_code == target_code:
                ...
                if not df_baseline.empty:
                    df_target_subset = df_historical[['report_date', target_price_col]].rename(columns={target_price_col: 'Target_Price'})
                    df_base_subset = df_baseline[['report_date', base_price_col]].rename(columns={base_price_col: 'Base_Price'})
                    ...
                    fig_spread.add_trace(go.Scatter(x=df_spread['report_date'], y=df_spread['Target_Price'], name=selected_cut_name, line=dict(color='blue')))
                    fig_spread.add_trace(go.Scatter(x=df_spread['report_date'], y=df_spread['Base_Price'], name=baseline_name, line=dict(color='red')))
                    fig_spread.update_layout(title="Historical Price Comparison", yaxis_title="Price ($/lb)", hovermode="x unified")
                    st.plotly_chart(fig_spread, use_container_width=True)
                    
                    st.markdown("##### The Dollar Spread Over Time")
                    fig_diff = go.Figure(data=[go.Bar(x=df_spread['report_date'], y=df_spread['Spread'], marker_color='green')])
